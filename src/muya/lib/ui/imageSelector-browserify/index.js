...
class ImageSelector extends BaseFloat {
  ...
  replaceImageAsync = async ({ alt, src, title,imgData }) => {
    //URL or has imageAction
    if (!this.muya.options.imageAction || URL_REG.test(src)) {
      ...
      if (imgData) {
        //try to add image path map
        try {
          let newSrc = await this.muya.options.imageAction(src)
          console.log(newSrc);
          newSrc = getImageSrc(newSrc).src
          //Only image data can be obtained in the browser
          if (imgData) {
            //this.muya.contentState.stateRender.urlMap.set(newSrc, imgData)
            this.muya.contentState.stateRender.loadImageMap.set(newSrc, imgData)
          }
  ...
  async handleSelectButtonClick () {
    //In the browser, you can only get the image data through the input tag
